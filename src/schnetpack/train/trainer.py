...
class Trainer:
    ...
    def __init__(
        ...
        if os.path.exists(self.checkpoint_path): self.restore_checkpoint()
        else:
    ...
    def _load_model_state_dict(self, state_dict):
        if self._check_is_parallel(): self._model.module.load_state_dict(state_dict)
        else:                         self._model.load_state_dict(state_dict)
    ...
    def state_dict(self):
        ...
        if self._check_is_parallel():  state_dict["model"] = self._model.module.state_dict()
        else:                          state_dict["model"] = self._model.state_dict()
        return state_dict
    ...
    def state_dict(self, state_dict):
        ...
        for h, s in zip(self.hooks, self.state_dict["hooks"]): h.state_dict = s
    ...
    def store_checkpoint(self):
        ...
        if len(chpts) > self.keep_n_checkpoints:
            ...
            for i in sidx[: -self.keep_n_checkpoints]: os.remove(os.path.join(self.checkpoint_path, chpts[i]))
    ...
    def train(self, device, n_epochs=sys.maxsize):
        ...
        for h in self.hooks: h.on_train_begin(self)
        ...
        try:
            for _ in range(n_epochs):
                ...
                for h in self.hooks: h.on_epoch_begin(self)
                ...
                for train_batch in train_iter:
                    ...
                    for h in self.hooks: h.on_batch_begin(self, train_batch)
                    ...
                    for h in self.hooks: h.on_batch_end(self, train_batch, result, loss)

                    if self._stop: break

                if self.epoch % self.checkpoint_interval == 0: self.store_checkpoint()

                # validation
                if self.epoch % self.validation_interval == 0 or self._stop:
                    for h in self.hooks: h.on_validation_begin(self)
                    ...
                    for val_batch in self.validation_loader:
                        ...
                        for h in self.hooks: h.on_validation_batch_begin(self)
                        ...
                        if self.loss_is_normalized: val_loss += val_batch_loss * vsize
                        else:                       val_loss += val_batch_loss

                        for h in self.hooks: h.on_validation_batch_end(self, val_batch, val_result)

                    # weighted average over batches
                    if self.loss_is_normalized: val_loss /= n_val

                    if self.best_loss > val_loss:
                        self.best_loss = val_loss
                        torch.save(self._model, self.best_model)

                    for h in self.hooks: h.on_validation_end(self, val_loss)

                for h in self.hooks: h.on_epoch_end(self)
            ...
            for h in self.hooks: h.on_train_ends(self)
            self.store_checkpoint()

        except Exception as e:
            for h in self.hooks: h.on_train_failed(self)
            raise e
